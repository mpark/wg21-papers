...
order to accomplish this, we need to do three things:
...
tuple-like (including aggregate) types.  The tuple protocol is used in
defining the behavior of structured bindings and pattern matching both.
...
This is what the `@*tuple-like*@` concept does.  The standard says, "A type
`T` models and satisfies the exposition-only concept *tuple-like* if
